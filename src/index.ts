...
import { FuzzySearchResponse, isSuccessResponse, ResultType } from './tomtom/types';
...
export async function getAutoCompleteDetails({ address, options }: { address: string, options?: AutoCompleteOptions }): Promise<AutoCompleteResult> {
    ...
    if (!apiKey || !apiBaseUrl || !apiVersion) {
        throw new MissingConfigError('Missing TomTom API configuration.  Have you set up your .env file?')
    }

    if (address.length < 1) {
        throw new InvalidArgumentError({ message: 'Address cannot be empty.', field: 'address' })
    }
